    ...
    return (
      <div className="min-h-screen bg-earth-beige flex flex-col items-center justify-center p-4 text-center font-sans">
        ...
        <div className="relative w-24 h-24 mb-8 flex items-center justify-center">
          ...
          <div className="absolute inset-2 rounded-full border-4 border-earth-rust/20 border-t-earth-rust animate-spin" style={{ animationDuration: '1.5s' }}></div>
          {/* Inner fast spinner */}
          <div className="absolute inset-6 rounded-full border-4 border-transparent border-t-earth-maroon border-b-earth-maroon animate-spin" style={{ animationDuration: '0.8s' }}></div>
          {/* Center core */}
        ...
        <p className="text-sm font-medium text-earth-maroon/70 max-w-xs leading-relaxed">
          We use a free hosting service that goes to sleep when inactive. 
          It might take <span className="font-bold text-earth-rust">up to a minute</span> for things to get fully ready. 
          <br /><br />
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route 
          path="/" 
          element={<Navigate to={isAuthenticated ? PATHS.DASHBOARD : PATHS.LOGIN} replace />} 
        />
        
        {/* Public Routes */}
        <Route path={PATHS.LOGIN} element={<AuthPage initialMode="login" />} />
        <Route path={PATHS.REGISTER} element={<AuthPage initialMode="register" />} />
