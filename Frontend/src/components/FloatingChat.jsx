  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (error.response?.status === 429) {
        ...
        const errorMessage = error.response?.data?.message || "Daily AI query limit reached for your session.";
        setMessages((prev) => [
          ...prev,
        ...
        setMessages((prev) => [
          ...prev,
          { role: "error", content: "❌ Something went wrong connecting to the AI." }
        ]);
      }
    } finally {
      setIsLoading(false);
    ...
    <div className="fixed bottom-6 right-6 z-50">
      ...
      {isOpen && (
        <div className="absolute bottom-16 right-0 w-[90vw] sm:w-96 bg-earth-beige border border-earth-rust/30 rounded-xl shadow-2xl flex flex-col overflow-hidden mb-4 transition-all duration-300 h-[500px] max-h-[75vh]">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-white/50">
                  ...
                  {msg.role === "user" ? (
                    ...
                    <ReactMarkdown 
                      ...
                      components={{
                        ...
                        h3: ({ node, ...props }) => <h3 className="font-bold text-base mt-2 mb-1 text-earth-maroon" {...props} />,
                        ...
                        strong: ({ node, ...props }) => <strong className="font-semibold text-earth-maroon" {...props} />,
                      }}
                    >
                      {msg.content}
                    </ReactMarkdown>
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-white border border-earth-rust/20 text-earth-maroon/70 rounded-2xl rounded-bl-none px-4 py-2 text-sm flex gap-1 items-center shadow-sm">
          ...
          {/* Input Area */}
          <form onSubmit={handleSendMessage} className="p-3 bg-white border-t border-earth-rust/20">
            <div className="flex gap-2">
              <input
                ...
                placeholder={isLimitReached ? "Daily limit reached..." : "Ask something..."}
                className="flex-1 bg-earth-beige/50 border border-earth-rust/30 rounded-lg px-3 py-2 text-sm text-earth-maroon focus:outline-none focus:ring-1 focus:ring-earth-rust disabled:opacity-50"
              />
              <button
                type="submit"
